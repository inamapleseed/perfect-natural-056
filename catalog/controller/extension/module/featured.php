...
class ControllerExtensionModuleFeatured extends Controller {
	...
	public function index($setting) {
		...
		if( isset($setting['myurl'][(int)$this->config->get('config_language_id')]) ){
			$data['myurl'] = $setting['myurl'][(int)$this->config->get('config_language_id')];
		}

		if( isset($setting['mydescription'][(int)$this->config->get('config_language_id')]) ){
			$data['mydescription'] = $setting['mydescription'][(int)$this->config->get('config_language_id')];
		}

		if( isset($setting['myurl_title'][(int)$this->config->get('config_language_id')]) ){
			$data['myurl_title'] = $setting['myurl_title'][(int)$this->config->get('config_language_id')];
		}
		...
		if (!empty($setting['product'])) { 
			...
			foreach ($products as $product_id) {
				$data['products'][] = $this->load->controller('component/product_info2', $product_id);
			}
		}
