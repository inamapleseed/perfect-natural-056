	...
	public function createCategories( & $core, $idx, $config ) {
		...
		foreach( $config as $key => $category ) {
			...
			switch( $category['type'] ) {
				case 'tree' : {
					if( NULL != ( $row['categories'] = $core->getTreeCategories( $idx, null, 'tree' ) ) || $this->rget('mfp_path') || ! empty( $category['show_go_back_to_top'] ) ) {
						$row['top_path'] = 0;
				...
				case 'cat_checkbox' : {
					...
					if( NULL == ( $row['categories'] = $core->getTreeCategories( $idx, $this->rget('mfp_org_path') ? $this->rget('mfp_org_path') : NULL, 'checkbox' ) ) ) {
						$row = NULL;
					}
