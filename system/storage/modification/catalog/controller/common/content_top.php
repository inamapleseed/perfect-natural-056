	...
	class ControllerCommonContentTop extends Controller {
		...
		private function encapsulate(&$output, $mode="", $module="", $background = ''){
			...
			if( is_file ( DIR_IMAGE . $background) ){
				...
				$style = 'data-bg="image/'.$background.'" style="background-color: transparent; background-position: center center; background-size: cover;"';
				$cls .= 'lazy';
			}

			if($mode == "full-width"){
				$output = "<div class='section-space max-offset $module $cls' $style>" . $output . "</div>";
			}
			elseif($mode == "full-width-with-container"){
				$output = "<div class='section-space max-offset $module $cls' $style><div class='container'>" . $output . "</div></div>";
			}else{
				$output = "<div class='section-space $module $cls' $style>" . $output . "</div>";
			}
		}
	}
