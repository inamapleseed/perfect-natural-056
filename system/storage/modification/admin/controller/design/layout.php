...
class ControllerDesignLayout extends Controller {
	...
	protected function getForm() {
		...
		foreach ($layout_modules as $layout_module) {
			...
			$background_thumb = $data['placeholder'];

			if(is_file(DIR_IMAGE . $layout_module['background'])) {
				
				$background = $layout_module['background'];
				
				$background_thumb = $this->model_tool_image->resize($background, 100, 100);
			...
			if (!isset($part[1])) {
				$data['layout_modules'][] = array(
					...
					'mode'       => $layout_module['mode'],
					'edit'       => $this->url->link('extension/module/' . $part[0], 'token=' . $this->session->data['token'], true),
					'position'   => $layout_module['position'],
					'sort_order' => $layout_module['sort_order'],

					'background' => $background,
					'background_thumb' => $background_thumb,
				);
			} else {
				$module_info = $this->model_extension_module->getModule($part[1]); //debuginfo($module_info);
