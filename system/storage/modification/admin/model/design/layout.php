	...
	public function addLayout($data) {
		...
		if (isset($data['layout_module'])) {
			foreach ($data['layout_module'] as $layout_module) {
				if(!isset($layout_module['mode'])) $layout_module['mode'] = '';

				if(!isset($layout_module['background'])) $layout_module['background'] = '';

				$this->db->query("INSERT INTO " . DB_PREFIX . "layout_module SET layout_id = '" . (int)$layout_id . "', mode = '" . $this->db->escape($layout_module['mode']) . "', code = '" . $this->db->escape($layout_module['code']) . "', position = '" . $this->db->escape($layout_module['position']) . "', sort_order = '" . (int)$layout_module['sort_order'] . "', background='" . $this->db->escape($layout_module['background']) . "'");
			}
		}
