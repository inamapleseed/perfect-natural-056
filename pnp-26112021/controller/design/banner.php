	...
	protected function getForm() {
		...
		foreach ($banner_images as $key => $value) {
			foreach ($value as $banner_image) {
				...
				if (is_file(DIR_IMAGE . $banner_image['design_image'])) {
					$design_image = $banner_image['design_image'];
					$design_thumb = $banner_image['design_image'];
				} else {
					$design_image = '';
					$design_thumb = 'no_image.png';
				}
				...
				$data['banner_images'][$key][] = array(
					...
					'design_thumb' => $this->model_tool_image->resize($design_thumb, 100, 100),
