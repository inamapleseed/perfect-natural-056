...
class ModelCatalogPagebanner extends Model {
	...
	public function addPageBanner($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "page_banner SET page_name = '" . $this->db->escape($data['page_name']) . "', page_description = '" . $this->db->escape($data['page_description']) . "',
	...
	public function editPageBanner($pb_id, $data) {
		...
		page_description = '" . $this->db->escape($data['page_description']) . "', 
		status = '" . (int)$data['status'] . "', 
		route = '" . $this->db->escape($data['route']) . "',
		query = '" . $this->db->escape($data['query']) . "',
		image = '" . $this->db->escape($data['image']) . "',
